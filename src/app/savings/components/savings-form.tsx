...
export function SavingsForm({ open, onOpenChange, addSavingsItem, updateSavingsItem, editingItem }: SavingsFormProps) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (editingItem) {
      ...
      addSavingsItem(submissionData as Omit<SavingsItem, 'id' | 'monthlyAmount'>);
    }
    onOpenChange(false);
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {editingItem ? 'Update the details for this fund.' : 'Create a new fund to save for future expenses.'}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
                    ...
                    <FormField
                        ...
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                            <div className="space-y-0.5">
                                <FormLabel>Custom Monthly Goal</FormLabel>
                            </div>
                            <FormControl>
                            <Switch
                                checked={field.value}
